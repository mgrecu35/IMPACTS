...
for i,Nbins in enumerate(nbinsL):
    ...
    if iwc>0:
        ...
        dm_mgL.append(dm)
        iwc_mgL.append(iwc)
        Nw=4**4/np.pi*iwc/(0.1*dm)**4/(rhow*1e3)
        nwL.append(Nw)
        rrateL.append(rrate)
    else:
        zKuL.append(-999)
...
import xarray as xr
dm_mgLX=xr.DataArray(dm_mgL,attrs=dict(description="Dm from PSD using HB",units="mm"))
nwLX=xr.DataArray(nwL,attrs=dict(description="Nw",units="cm^{-4}"))
...
iwc_mgLX=xr.DataArray(iwc_mgL,attrs=dict(description="iwc from PSD using HB",units="g/m^3"))
rrateLX=xr.DataArray(rrateL,attrs=dict(description="flux from PSD using HB",units="g/m^3"))
#--------------------------------------------------------------------------------#
...
ds=xr.Dataset({"dm":dmass_all,"iwc_all":iwc_all,"iwc_wisper":iwc_wisperX,\
               ...
               "iwc_PSD":iwc_mgLX,"dm_PSD":dm_mgLX,"Nw":nwLX,"precipRate":rrateLX})
