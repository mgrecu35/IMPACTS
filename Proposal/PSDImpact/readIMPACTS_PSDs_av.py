...
def calcProp(Nbins,ds,mass_av,vol_av,dbinsM,K2r,xf1,xf2,xf3,K2,ik,gradZ,graddm,gradiwc,gradrho,dNbins):
    mass_avg=mass_av.mean(axis=-1)
    vol_avg=vol_av.mean(axis=-1)
    mass_avg2=mass_av[:,20:].mean(axis=-1)
    vol_avg2=vol_av[:,20:].mean(axis=-1)
    sback13_avg=sback13.mean(axis=-1)
    sback37_avg=sback37.mean(axis=-1)
    sback95_avg=sback95.mean(axis=-1)
    sback13_avg2=sback13[:,20:].mean(axis=-1)
    sback37_avg2=sback37[:,20:].mean(axis=-1)
    sback95_avg2=sback95[:,20:].mean(axis=-1)
    kext_ka_avg=kext_ka_av.mean(axis=-1)
    kext_w_avg=kext_ka_av.mean(axis=-1)
    kext_ku_avg=kext_ka_av.mean(axis=-1)
    
    dm=sum(Nbins*ds*mass_avg*dbinsM)/sum(Nbins*ds*mass_avg)
    iwc_ps=sum(Nbins*ds*mass_avg)*1e-3
    rho=sum(Nbins*ds*mass_avg)/sum(Nbins*ds*vol_avg)
    ...
    Z1_dda=log10(sum(Nbins*1e-6*ds*sback13_avg/K2))*10.
    Z3_dda=log10(sum(Nbins*1e-6*ds*sback95_avg/K2))*10.
    Z2_dda=log10(sum(Nbins*1e-6*ds*sback37_avg/K2))*10.
    
    gradZ[42,0]=log10(sum(Nbins*1e-6*ds*sback13_avg2/K2))*10.
    gradZ[42,2]=log10(sum(Nbins*1e-6*ds*sback95_avg2/K2))*10.
    gradZ[42,1]=log10(sum(Nbins*1e-6*ds*sback37_avg2/K2))*10.

    gradiwc[42]=sum((Nbins+dNbins)*ds*mass_avg2)*1e-3
    graddm[42]=sum((Nbins+dNbins)*ds*mass_avg2*dbinsM)/sum((Nbins+dNbins)*ds*mass_avg2)
    gradrho[42]=sum((Nbins+dNbins)*ds*mass_avg2)/sum((Nbins+dNbins)*ds*vol_avg2)
        
    for i in range(42):
        dNbins*=0
        dNbins[i]=Nbins[i]*0.1
        gradZ[i,0]=log10(sum((Nbins+dNbins)*1e-6*ds*sback13_avg/K2))*10.
        gradZ[i,1]=log10(sum((Nbins+dNbins)*1e-6*ds*sback37_avg/K2))*10.
        gradZ[i,2]=log10(sum((Nbins+dNbins)*1e-6*ds*sback95_avg/K2))*10.
        gradiwc[i]=sum((Nbins+dNbins)*ds*mass_avg)*1e-3
        graddm[i]=sum((Nbins+dNbins)*ds*mass_avg*dbinsM)/sum((Nbins+dNbins)*ds*mass_avg)
        gradrho[i]=sum((Nbins+dNbins)*ds*mass_avg)/sum((Nbins+dNbins)*ds*vol_avg)
    kext35=4.34*sum(Nbins*ds*1e-6*kext_ka_avg)*1e3
    kext95=4.34*sum(Nbins*ds*1e-6*kext_w_avg)*1e3
    kext13=4.34*sum(Nbins*ds*1e-6*kext_ku_avg)*1e3
...
ds.to_netcdf("IMPACTS_PSD_DatabaseAvg.nc",encoding=encoding)
