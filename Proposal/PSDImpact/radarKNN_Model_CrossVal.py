...
import matplotlib
d=pickle.load(open("simulatedPropTempSorted.pklz","rb"))
zSims=d["zSims"]
kextL=d["kextL"]
rhoL=d["rhoL"]
...
d=pickle.load(open("simulatedPropTempSorted_OLYMPEX.pklz","rb"))
zSims_O=d["zSims"]
kextL_O=d["kextL"]
rhoL_O=d["rhoL"]
tempC_O=np.array(d["tempC"])
kextL_O=np.array(kextL_O)
...
def getXY(zSims,tempC,rhoL,kextL):
    a=np.nonzero(zSims[:,0]<40)
    zSims=zSims[a[0],:]
    tempC=tempC[a[0]]
    rhoL=np.array(rhoL)[a]
    kextL=kextL[a[0],:]
    n=zSims.shape[0]
    ...
    X[:,2]=(X[:,2]-2)/7
    X[:,3]=(X[:,3]+11)/8.0
    return X,Y,Yice,Yattw,Yrho,Yzw
...
if iopt==1:
    ...
    X_train,y_train,yice_train,yattw_train,yrho_train,yzw_train=getXY(zSims,tempC,rhoL,kextL)
    X_test,y_test,yice_test,yattw_test,yrho_test,yzw_test=getXY(zSims_O,tempC_O,rhoL_O,kextL_O)
...
neigh_rho.fit(X_train[:,0:2], yrho_train)
...
yp_rho=neigh_rho.predict(X_test[:,0:2])
...
pickle.dump({"knn_dm":neigh,"knn_iwc":neigh_iwc, "knn_zw":neigh_zw,\
             ...
             "knn_rho":neigh_rho},\
            open("knnModelsIMPACTS_2_DDA.pklz","wb"))
plt.close('all')
import matplotlib
