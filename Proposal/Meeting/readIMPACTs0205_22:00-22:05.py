...
plt.colorbar()
plt.subplot(212)
plt.pcolormesh(time[a[0]],hm-rmean[:192*3],(zKa_slice[:,:192*3]).T,cmap='jet',vmin=0,vmax=30)
plt.xlim(time[a[0][0]],time[a[0][-1]])
...
info1,info2=getinfo(t1+0.05,t2+0.1)
...
knn_iwc=d["knn_iwc"]
knn_zw=d["knn_zw"]
dm_ret=zKa_slice[:,300:(192*3-35)].copy()*0
iwc_ret=zKa_slice[:,300:(192*3-35)].copy()*0
zw_ret=zKa_slice[:,300:(192*3-35)].copy()*0
dfr_knn=(zKu_slice[:,300:(192*3-35)]-zKa_slice[:,300:(192*3-35)])
zku_knn=zKu_slice[:,300:(192*3-35)]
...
plt.figure()
plt.pcolormesh(time[a][:],hm-rmean[300:(192*3-35)],iwc_ret.T,cmap='jet',vmin=0,vmax=1.2)
plt.title('Retrieved IWC\n 05 February 2020')
plt.ylabel('Height(m)')
plt.xlabel('Time')
cbar=plt.colorbar()
cbar.ax.set_title('g/m^3')
plt.savefig('retIWC_05Feb2020_22:00-22:05.png')
...
aC=np.nonzero((timeCRS[:,0]-t1)*(timeCRS[:,0]-t2)<0)
...
for i1 in a[0][:]:
    ...
    zwret1=np.interp(hint,(hm-rmean[300:(192*3-35)])[::-1],zw_ret[i1-a[0][0],::-1])
    for k in range(54):
...
plt.colorbar()
plt.subplot(212)
plt.pcolormesh(time[a][:],hm-rmean[300:(192*3-35)],(zw_ret).T,cmap='jet',vmin=0,vmax=25)
plt.title('Retrieved Zw\n 05 February 2020')
plt.ylabel('Height(m)')
plt.xlabel('Time')
cbar=plt.colorbar()
cbar.ax.set_title('g/m^3')
plt.savefig('retZw_05Feb2020_22:00-22:05.png')
