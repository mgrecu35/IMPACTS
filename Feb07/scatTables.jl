...
for i=1:nT
    dmT[i]=0.75*(dmT[1]+0.75*(dmT[i]-dmT[1]))*1.2
...
function getDmNwSF(tempKu::Array{Float32,1},massKu::Array{Float32,2},fractionKu::Array{Float32,1},
    ...
    for i=1:nx
        ...
        get_rZs(rwc,ns1,bscat,scat,ext,g,Deq,Deq_r,vfall,vfall_r,wlKa,dnRet,mu)
        zKu1, attKu1,scatIntKu1,gIntKu1, vdopKu1,dmKu1,rrate1=
        get_rZs(rwc,ns1,bscatKu,scatKu,extKu,gKu,DeqKu,DeqKu_r,vfallKu,vfallKu_r,wlKu,dnRet,mu)
        println("zsims=$(zKu1) $(Z) $(zObs1)")
        zW, attW,scatIntW,gIntW, vdopW,dmW,rrate=
        get_rZs(rwc,ns1,bscatW,scatW,extW,gW,DeqW,DeqW_r,vfallW,vfallW_r,wlW,dnRet,mu)
        rateTs[i]=rrate
        #println("$Z $(zKu[i]) $(dm) $(dmc)")
        zKaTs[i]=zKa
